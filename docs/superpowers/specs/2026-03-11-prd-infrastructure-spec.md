...
- **Centralize Configuration:** Promote service ports, hostnames, and flags to canonical variables in `ansible/group_vars/all/all.yml`.
- **Template Generation:** Generate configuration files (Grafana datasources, Prometheus targets, Homepage widgets) using Jinja2 templates via Ansible, bypassing setup wizards.
...
- Full IaC templating of *every* service config (e.g., Uptime Kuma REST API setup is deferred).
